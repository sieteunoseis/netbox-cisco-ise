"""Mark the unmanaged CiscoIse permission-anchor model as private.

The model exists only to attach the `configure_ciscoise` permission and has
...
def mark_stub_model_private(apps, schema_editor):
    ...
    try:
        ct = ContentType.objects.get(app_label="netbox_cisco_ise", model="ciscoise")
